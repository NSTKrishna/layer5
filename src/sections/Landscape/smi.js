...
function SMI_Compatibility() {
    ...
    fetchJson("https://cloud.layer5.io/api/smi/results/public")
      ...
      .then(results => {
        if (!Array.isArray(results?.results)) {
          throw new Error("Unexpected response shape from SMI conformance results API");
        }
  ...
  if (loadError) {
    return <div className="landscape-table">Unable to load SMI conformance results right now. Please try again later.</div>;
  }
